...
export default function FastingCountdown({ end, duration }: InitialData) {
  ...
  function calculateTimeLeft() {
    ...
    seconds.value = Math.floor(difference / 1000) - hours.value * 60 * 60 -
      minutes.value * 60;
  }
        ...
        <div className="m-4 w-full">
          <div className="relative pt-1">
            ...
            <div className="border-2 border-green-300 overflow-hidden h-4 m-4 text-xs flex rounded bg-green-200">
              <div
                style={{ width: `${progressToStart.value}%` }}
    ...
    <div className="flex flex-col items-center justify-center w-full">
      <div className="m-4 w-full">
        <div className="relative pt-1">
          ...
          <div
            className="m-4 grid gap-1 justify-center mx-auto"
            style={{ gridTemplateColumns: `repeat(5, 30px)`, width: "165px" }}
          >
            {Array.from({ length: 5 * 5 }).map((_, index) => {
              const cellProgress = (index + 1) * 4;
              const isFilled = cellProgress <= progress.value;
              const isNext = cellProgress > progress.value &&
                cellProgress <= progress.value + 4;
        ...
        <div className="mt-4 text-center text-sm">
          {progress.value >= 99 && progress.value != 100 && (
            <p>
              Letzten Schritte sind zu erledigen! Gekocht? Gedeckt? Bestellt?
              Guten Appetit!
            </p>
          )}

          {progress.value >= 75 && progress.value < 99 &&
            progress.value != 100 && (
            <p>Respekt! Jetzt stark bleiben, bald kannst du futtern!</p>
          )}

          {progress.value >= 50 && progress.value < 75 &&
            progress.value != 100 && (
            <p>Durchhalten! Du hast schon mehr als die Hälfte geschafft.</p>
          )}

          {progress.value >= 25 && progress.value < 50 &&
            progress.value != 100 && (
